        ...
        if form.is_valid():
            if not User.objects.filter(username=request.POST['documento']):
                ...
                if request.POST['rol'] == 'Administrador':
                    ...
                    admin_group = Group.objects.get(name='usuario_contraloria')
                    user.groups.add(admin_group)
...
def usuarios_editar(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.POST['rol'] == 'Administrador':
                admin_group = Group.objects.get(name='Administrador')
                usuario.user.groups.add(admin_group)
            else:
                admin_group = Group.objects.get(name='usuario_contraloria')
                usuario.user.groups.add(admin_group)
            return redirect('usuarios')
...
def usuarios(request):
    ...
    rol_usuario = Usuario.objects.filter (user = request.user.id).first() 
    listar_usuario=Usuario.objects.filter(Q(estado='1')|Q(estado='0'),Q(rol='Administrador')|Q(rol='contraloria'))
