    ...
    if request.method == 'POST':
        ...
        if info_software.is_valid():
            ...
            if new_info_software:
                ...
                for field_name, field_value in info_software.cleaned_data.items():
                    setattr(new_info_software, field_name, field_value)
    ...
    if request.method == 'POST':
        ...
        if info_hardware.is_valid():
            ...
            if new_info_hardware:
                ...
                for field_name, field_value in info_hardware.cleaned_data.items():
                    setattr(new_info_hardware, field_name, field_value)
        ...
        formulario = InformacionHardwareForm(instance=existing_info_hardware)

        consulta = TipoActivo.objects.filter(id=id)
        context = {'form': formulario, 'consulta': consulta, 'rol_usuario':rol_usuario}
...
class generar_pdf(View):
    def get(request,self,pk):  
        ...
        version_ofimatica = VersionOfimatica.objects.get(id=software.version_ofimatica.id)
        antivirus = Antivirus.objects.get(id=software.antivirus.id)
        herramienta_cloud = HerramientaCloud.objects.get(id=software.herramienta_cloud.id)
        navegador_1 = Navegador.objects.get(id=software.navegador_1.id)
        navegador_2 = Navegador.objects.get(id=software.navegador_2.id)

        ofimatica = Ofimatica.objects.get(id=software.ofimatica.id)
        ...
        data = {
                ...
                'antivirus': antivirus,
                'navegador_1':navegador_1,
                'navegador_2':navegador_2,
                'ofimatica':ofimatica,
