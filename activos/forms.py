...
class InformacionSoftwareForm(forms.ModelForm):
    ...
    """Este codigo sirve para filtrar el segundo input de version de sistemaOperativo por su correspondiente sistemaOperativo dentro del template de informacionsoftware
    deseaba que mostrara los diferentes versiones de acuerdo a el cambio de su sistemaoperativo
        def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if 'sistema_operativo' in self.data or 'ofimatica' in self.data :
            try:
                id_ofimatica = int(self.data.get('ofimatica'))
                id_sistema = int(self.data.get('sistema_operativo'))
                self.fields['version_sistema_op'].queryset = VersionSistemaOperativo.objects.filter(sistema_operativo_id=id_sistema)
                self.fields['version_ofimatica'].queryset = VersionOfimatica.objects.filter(paquete_id=id_ofimatica)
            except (ValueError, TypeError):
                pass
        elif self.instance.pk:
            self.fields['version_sistema_op'].queryset = self.instance.sistema_operativo.versionsistemaoperativo_set.order_by('version_sistema')
            self.fields['version_ofimatica'].queryset = self.instance.ofimatica.versionofimatica_set.order_by('version')"""
