...
class TipoActivoForm(ModelForm):
    class Meta:
        model = TipoActivo
        exclude=['user', 'estado_software','estado_hardware']
...
class InformacionSoftwareForm(ModelForm):
    class Meta:
        ...
        exclude=['id_activo', 'registrado', 'user']
    sistema_operativo = forms.ModelChoiceField(queryset = SistemaOperativo.objects.all())
